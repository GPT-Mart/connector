...
async function startServer() {
  ...
  const HOST = '0.0.0.0'; // Render/containers
  ...
  async function readDB() {
    try {
      ...
      const formattedGpts = SEED_GPTS.map(item => ({
        id: uuidv4(),
  ...
  // simple IP rate-limit store for submit/lead
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      // Seed defaults (merge; dedupe by URL)
        ...
        for (const s of SEED_GPTS) {
          if (!byUrl.has(s.url)) {
            const item = {
              ...
              title: s.title, desc: s.desc, icon: s.icon,
              categories: s.categories || [], tags: s.tags || [], url: s.url
            };
    ...
    // Static files (for local testing / simple hosting)
    try {
